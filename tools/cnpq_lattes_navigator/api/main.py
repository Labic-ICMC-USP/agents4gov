...
class CommitteeMember(BaseModel):
    ...
    role: str  # "advisor", "internal", "external", "substitute"
    is_president: bool = False


class CommitteeValidationRequest(BaseModel):
    student: Researcher
    advisor: Researcher
    committee_members: List[CommitteeMember]
    thesis_title: Optional[str] = None
    committee_type: str = "qualification"  # "qualification", "defense"
    time_window: int = 5
    coi_rules: Optional[dict] = None
...
@app.post("/validate-committee")
def validate_committee(request: CommitteeValidationRequest):
    if not tool:
        return {"status": "error", "message": "Tool not initialized", "import_error": browser_import_error}
    
    student_data = request.student.model_dump()
    advisor_data = request.advisor.model_dump()
    members_data = [m.model_dump() for m in request.committee_members]
    coi_config = request.coi_rules or {"R1": True, "R2": True, "R3": True, "R4": True, "R5": True, "R6": True, "R7": True}
    
    result = tool.validate_committee(
        student=student_data,
        advisor=advisor_data,
        committee_members=members_data,
        time_window=request.time_window,
        coi_rules_config=coi_config
    )
